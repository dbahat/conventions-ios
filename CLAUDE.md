...
**SwiftUI migration (in progress).** The app is being migrated from UIKit to SwiftUI incrementally, one screen at a time — existing storyboard-driven `UIViewController`s are kept as thin hosts wired into `Main.storyboard`/navigation, and their content is replaced with a SwiftUI `View` embedded via `UIHostingController`, rather than rewriting screens as a SwiftUI `App`/`Scene`. First example: `moreinfo/MoreInfoViewController.swift` still is the storyboard-addressable `UIViewController` (handles navigation via `select(_:)`), but its `embedSwiftUIContent()` hosts `moreinfo/MoreInfoView.swift` (a SwiftUI `View`) via `UIHostingController`. Follow this same host/embed pattern for future screen migrations. The `stands/` feature (see below) is the most fully-SwiftUI screen built this way so far: every screen in it is a UIKit host embedding a SwiftUI view.
