...
**Convention-centric singleton model.** `model/Convention.swift` (`Convention.instance`) is the root object for all convention-specific state: dates, display name, hall list (with display order), feedback survey definitions (Google Forms), and owns the `Events`, `Updates`, `SecondHand`, `UserInputs`, and `stands` (`StandsRefresher`) singletons. Almost everything else in the app reads from `Convention.instance`. When a new yearly edition of the app is prepared, this file (name/slug/dates/halls/forms) is the primary place that changes.
...
**Data flow for stands (dealer's room / vendor booths):** mirrors the events pattern above, in its own `stands/` folder.
- `stands/StandsRefresher.swift` fetches from `https://api.sf-f.org.il/booths/<slug>.json` (unlike the events endpoint, this one is parameterized per-convention) and parses via `stands/StandsParser.swift`.
- Same cache/bundled-snapshot fallback as events: `Library/Caches/<name>Stands.json`, falling back to `Conventions/cache/<slug>Stands.json` bundled in the app.
- `stands/Stand.swift` is the parsed model (`id`, `name`, `category`, `area: StandArea?`, `description`, `tags`, `tableIds`, `discountOrga`, `dates`, `url`, `logo`); `getAll()` filters out any stand with a `nil` area. `StandArea` (`id`/`title`) groups stands into physical areas/halls of the dealer's room, distinct from `model/Hall.swift` (which is for event halls).
...
**SwiftUI migration (in progress).** The app is being migrated from UIKit to SwiftUI incrementally, one screen at a time — existing storyboard-driven `UIViewController`s are kept as thin hosts wired into `Main.storyboard`/navigation, and their content is replaced with a SwiftUI `View` embedded via `UIHostingController`, rather than rewriting screens as a SwiftUI `App`/`Scene`. The shared boilerplate for this (addChild/constrain-to-edges/didMove) lives in one place: `UIViewController.embedSwiftUIView(_:backgroundColor:)`, a `UIViewController` extension in `UIViewControllerExtentions.swift` (defaults `backgroundColor` to `.clear`; pass `nil` to leave the hosting controller's own background untouched, as `stands/StandsMapViewController.swift` does for its fullscreen opaque map). Each host's `viewDidLoad` builds its screen-specific SwiftUI `View` (often using `self` — e.g. mapping over state, wiring `[weak self]` closures back to private nav methods) and calls this helper; when that construction is more than a one-liner, it's kept in a private `embedSwiftUIContent()` method for readability, otherwise it's called directly from `viewDidLoad`. First example: `moreinfo/MoreInfoViewController.swift` still is the storyboard-addressable `UIViewController` (handles navigation via `select(_:)`), but its `embedSwiftUIContent()` hosts `moreinfo/MoreInfoView.swift` (a SwiftUI `View`). Follow this same host/embed pattern for future screen migrations. The `stands/` feature (see below) is the most fully-SwiftUI screen built this way so far: every screen in it is a UIKit host embedding a SwiftUI view.

**Stands (dealer's room) screens.** Reached from More Info → "דוכנים" (`moreinfo/MoreInfoViewController.swift`). Navigation: `StandAreasViewController` (list of areas with a search bar that switches into a flat, filterable stand search — see `StandAreasSearchState`) → `StandsListViewController` (stands within one area, plus an expandable area map) → `StandsMapViewController` (fullscreen pinch-zoom map, presented modally, temporarily forces landscape/portrait based on the map image's aspect ratio via `AppDelegate.orientationLock`). `StandsFilterViewController` is a separate pushed screen for category filtering, sharing the same `StandAreasSearchState` as the areas/search screen. `StandCardView` renders one stand (with optional search-term highlighting); `StandsMapViewController` is deliberately not shared with the Map tab's `MapFloorViewController` even though both do pinch-zoom floor plans, since the two maps are expected to diverge over time.
...
3. **Bundled events/stands cache** — add `cache/<newSlug>Events.json` and `cache/<newSlug>Stands.json`, remove the old ones; must be added/removed as an actual **Xcode resource reference** in `project.pbxproj` (drag into Xcode), not just changed on disk. No other code changes needed — `Events.swift`, `StandsRefresher.swift`, `UserTicketsRetriever.swift`, `MyEventsViewController.swift`'s UserDefaults keys, and `NotificationsSchedualer` are already parameterized by `Convention.name`/`slug`.
